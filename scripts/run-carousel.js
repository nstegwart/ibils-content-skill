...
async function writePlan(articles) {
  ...
  const prompt = [
    ...
    "CLEAR THE VALUE BAR in content-rules.md: the carousel must be relatable to",
    "an ordinary Indonesian's money life, teach ONE real lesson with its WHY,",
    "and leave a clear takeaway. The cover is a scroll-stopping hook, not a flat",
    "label. For education mode, anchor the lesson in a real, well-known",
    "finance-book idea (named plainly). Before writing, reject any angle a real",
    "person would not find useful or would not send to a friend.",
